...
```java
// 区间[left, right]被划分成[left, mid]和[mid + 1, right]： 
// 称之为左二分查询，查找左侧第一个满足条件的数
int leftBinarySearch(int left, int right) {  
   while (left < right) {  
...
// 区间[left, right]被划分成[left, mid - 1]和[mid, right]：  
// 称之为右二分查询，查找右侧最后一个满足条件的数
int rightBinarySearch(int left, int right) {  
   while (left < right) {
