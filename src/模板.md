...
**拉链法**
开一个一维数组（例如 0 ~ $10^5 - 1$）存储所有的值，数组中的每一位存储 e[ ] 和 ne[ ]，因为把每一位看作一个槽，里面放一个链表，理想的情况下每个槽只有一个数，冲突的时候会需要链表。
**添加**和**查找**：h(x)
...
```java
// 对于每个点k，开一个单链表，存储k所有邻接点；h[k]存储这个单链表的头结点
int h[N], e[N], ne[N], idx;
